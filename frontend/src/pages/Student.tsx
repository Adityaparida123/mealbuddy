...
import { isGeminiConfigured } from '../../../shared/src/services/gemini';
  ...
  const header = (
    <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          {menuOpen ? 'Hide menu' : 'Todayâ€™s menu'}
        </button>
    ...
    <div className="flex h-screen flex-col">
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-3 overflow-hidden px-3 py-3 sm:px-4">
            ...
            {menu
              ...
              .map(m => (
                <span key={m.id} className="inline-flex items-center gap-1 rounded-full bg-orange-50 px-2.5 py-1 text-xs text-slate-600">
                  {m.name} <span className="font-semibold text-orange-600">â‚¹{m.price}</span>
                </span>
              ))}
          </div>
        )}
        ...
        <p className="text-center text-[11px] leading-relaxed text-slate-400">
          ...
          <span className="mx-1">Â·</span>Allergy safety is a hard constraint.
